HAS_MODEL=0
HAS_DATASET=0
HAS_PRETRAIN_VISION_PATH=0
for arg in "$@"; do
    case $arg in
        model=*) HAS_MODEL=1 ;;
        dataset=*) HAS_DATASET=1 ;;
        pretrain_vision_path=*) HAS_PRETRAIN_VISION_PATH=1 ;;
    esac
done

if (( ! HAS_MODEL || ! HAS_DATASET || ! HAS_PRETRAIN_VISION_PATH )); then
    echo "Usage: bash scripts/flexrank_cv.sh model=<model> dataset=<dataset> pretrain_vision_path=<path> [key=value ...]"
    echo "Error: model, dataset, and pretrain_vision_path are required"
    exit 1
fi
...
if command -v nvidia-smi &> /dev/null; then
    if [ -n "$CUDA_VISIBLE_DEVICES" ]; then
        # Count GPUs from CUDA_VISIBLE_DEVICES (e.g., "0,1,2")
...
usage() {
    ...
    echo "  pretrain_vision_path: directory containing pretrained vision checkpoints"
}
...
for arg in "$@"; do
    ...
    case $key in
        ...
        pretrain_vision_path) PRETRAIN_VISION_PATH=$value ;;
...
if [[ -z "$MODEL" || -z "$DATASET" || -z "$PRETRAIN_VISION_PATH" ]]; then
    echo "Error: model, dataset, and pretrain_vision_path are required"
    usage
    exit 1
fi
...
RUN_NAME=${DCMP}_${P_STR}_${S_STR}_${M_STR}_${DS_STR}_${LOSS_STR}_${LR_STR}
...
SCRIPT_ARGS="train.py \
    ...
    dataset=$DATASET \
    model=$MODEL \
    model.model_name_or_path=$PRETRAIN_VISION_PATH/${M_STR}_${DS_STR} \
    train.ce_loss_w=$CE \
    train.kl_loss_w=$KL \
